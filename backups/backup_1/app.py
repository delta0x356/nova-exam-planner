"""
Nova Exam Planner v4 — Multi-User Smart Study Scheduler
=======================================================

What's new in v4
----------------
* **Multi-user support** via SQLite + secure PBKDF2 password hashing.
* **External APIs**
    - Public holidays (date.nager.at) are auto-excluded from the plan.
    - A daily motivational quote (zenquotes.io) greets every user.
* **Redesigned UI**
    - Landing page with login / register, onboarding for new accounts.
    - Personalised dashboard with greeting, today's tasks, next exam, quote.
    - Refreshed theme with gradient cards and nicer empty states.
* **Better data model** — a SQL schema instead of three CSVs, with
  foreign keys and per-user isolation.
* **Extra goodies** — ICS export for your calendar app, profile page,
  password change.
"""
...
APP_TAGLINE = "Smart study scheduling that actually works."
APP_VERSION = "v4"

COURSE_COLORS = [
    "#4361ee", "#f72585", "#4cc9f0", "#7209b7", "#3a0ca3",
    "#f77f00", "#06d6a0", "#ef476f", "#118ab2", "#ffd166",
    "#073b4c", "#e63946",
]
...
.nova-hero h1 { color:white !important; margin:0 0 6px 0; font-size:1.8em; }
.nova-hero p  { opacity:0.9; margin:0; font-size:1.05em; }
...
.nova-quote small {
    display: block; margin-top: 10px; opacity: 0.85;
...
.nova-user-row .meta { line-height: 1.2; }
.nova-user-row .meta .hello { font-size: 0.78em; color: var(--muted); }
.nova-user-row .meta .name  { font-weight: 700; }
...
.nova-task .t-title { font-weight: 600; }
.nova-task .t-meta  { font-size: 0.85em; color: var(--muted); }
...
.chip-urgent  { background: #fee2e2; color: #b91c1c; }
...
/* Login container */
.nova-login-wrap h2 { text-align: center; margin-bottom: 6px; }
.nova-login-wrap .sub { text-align: center; color: var(--muted); margin-bottom: 18px; }
</style>
"""
...
def courses_total_minutes(user_id: int) -> dict[int, float]:
    """Map course_id → required total study minutes."""
...
def redistribute_missed_sessions(user: dict,
    ...
    for _, row in missed.iterrows():
        ...
        # Lock in anything actually completed and remove the missed remainder
        # from the past plan so completion metrics stay honest.
...
def page_auth():
    """Unauthenticated entry screen: tabs for login + register."""
    _, center, _ = st.columns([1, 2, 1])
    with center:
        st.markdown(
            f"""
            <div class="nova-hero" style="text-align:center;">
                <h1>🎓 {APP_TITLE}</h1>
                <p>{APP_TAGLINE}</p>
            </div>
            """,
        ...
        with tab_register:
            with st.form("register_form", clear_on_submit=False):
                ...
                country_idx = next(
                    (i for i, (code, _) in enumerate(countries) if code == "DE"),
                    0)
                ...
                rp = st.text_input("Password *", type="password",
                                   help="≥ 8 characters, with letters & digits")
        ...
        st.caption(
            ...
            "Data is local to this server only.")


# ═══════════════════════════════════════════════════════════════════════════════
# 4. DASHBOARD (new in v4)
# ═══════════════════════════════════════════════════════════════════════════════
...
def page_dashboard(user: dict):
    ...
    st.markdown(
        ...
        <div class="nova-hero">
            ...
            <p>Here's your study snapshot for {dt.date.today():%A, %d %B %Y}.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    # Quote of the day
    quote, author = api.fetch_daily_quote()
    st.markdown(
        f"""
        <div class="nova-quote">
            “{quote}”
            <small>— {author}</small>
        </div>
        """,
    ...
    # Onboarding when nothing set up yet
    if not courses:
        st.info(
            "🚀 **Let's get started.**  Head to **📚 Courses** in the sidebar "
            "to add your first course and generate a study plan.")
        _render_quickstart()
        return

    # KPIs
    total_planned   = int(sessions_df["planned_minutes"].sum()) if not sessions_df.empty else 0
    total_completed = int(sessions_df["completed_minutes"].sum()) if not sessions_df.empty else 0
    ...
    k2.metric("⏱️ Planned", fmt_minutes(total_planned))
    ...
    if today_tasks.empty:
        st.caption("No sessions today — enjoy the break 🌿")
    else:
        all_names = [c["name"] for c in courses]
        for _, row in today_tasks.iterrows():
            clr = course_color(row["course_name"], all_names)
            done = row["completed_minutes"] >= row["planned_minutes"] \
                and row["planned_minutes"] > 0
            klass = "nova-task done" if done else "nova-task"
            check = "✅ " if done else ""
            st.markdown(
                f"""
                <div class="{klass}" style="border-left-color:{clr};">
                    <div class="t-title">{check}{row['course_name']}</div>
                    <div class="t-meta">{fmt_minutes(row['planned_minutes'])} planned
                    {'· ' + fmt_minutes(row['completed_minutes']) + ' done'
                     if row['completed_minutes'] else ''}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        for c in upcoming:
            ...
            st.markdown(
                ...
                <div style="display:flex;align-items:center;margin-bottom:6px;">
                    <span class="nova-chip {chip_class}">{label}</span>
...
def _render_quickstart():
    ...
    st.markdown("### 3 steps to get going")
    c1, c2, c3 = st.columns(3)
    for col, num, emoji, title, body in [
        (c1, "1", "📚", "Add courses",
         "Name, exam date, ECTS and a difficulty from 1–5.  We'll estimate hours."),
        (c2, "2", "⚙️", "Set constraints",
         "Weekly hours, preferred weekdays, daily cap, and (optional) skip public holidays."),
        (c3, "3", "🚀", "Generate plan",
         "One click builds a balanced schedule that fits every exam deadline."),
    ]:
        with col:
            with st.container(border=True):
...
def page_courses(user: dict):
    st.header("📚 Courses & Constraints")

    # ── constraints ───────────────────────────────────────────────────────────
    st.subheader("⚙️ Study Constraints")
    con = db.get_constraints(user["id"])
    ...
    with c4:
        skip_holidays = st.checkbox(
            f"🏖️ Skip public holidays (country: {user['country_code']})",
    ...
    st.subheader("✏️ Edit course" if editing else "➕ Add course")
    ...
    for c in courses:
        ...
        icon = "🔴" if days_until <= 7 else "🟡" if days_until <= 21 else "🟢"

        with st.container(border=True):
            cols = st.columns([0.30, 0.10, 0.10, 0.16, 0.14, 0.10, 0.10])
            cols[0].markdown(f"**{icon} {c['name']}**")
            ...
            cols[5].caption(
                f"{days_until}d" if days_until >= 0 else "past")
            with cols[6]:
                b1, b2 = st.columns(2)
                if b1.button("✏️", key=f"edit_{c['id']}",
                             help="Edit this course"):
                    st.session_state["editing_course_id"] = c["id"]
                    st.rerun()
                if b2.button("🗑️", key=f"del_{c['id']}",
    ...
    if st.button("✨ Auto-generate balanced plan", type="primary",
                 use_container_width=True):
        with st.spinner("Building your schedule…"):
            # Persist constraints first so scheduling reflects what user sees
            db.save_constraints(
                user["id"],
                weekly_hours=weekly_hours,
                preferred_days=preferred_days,
        ...
        st.success(
            ...
            f"Head to **📅 Study Plan** to see it.")
...
def page_study_plan(user: dict):
    ...
    if not today_tasks.empty:
        ...
        for _, row in today_tasks.iterrows():
            ...
            st.markdown(
                ...
                <div class="{klass}" style="border-left-color:{clr};">
                    <div class="t-title">{check}{row['course_name']}</div>
                    <div class="t-meta">{fmt_minutes(row['planned_minutes'])} planned</div>
                </div>
                """, unsafe_allow_html=True)
        st.divider()
    ...
    for offset in range(7):
        ...
        with cols[offset]:
            header_bg = ("linear-gradient(135deg,#4361ee,#7209b7)" if is_today
                         else "var(--secondary-background-color)")
            header_fg = "white" if is_today else "inherit"
            st.markdown(
                f"""
                <div style="background:{header_bg};color:{header_fg};
            ...
            if day_data.empty:
                ...
                for _, row in day_data.iterrows():
                    ...
                    st.markdown(
                        f"""
                        <div style="border-left:3px solid {clr};
                                    padding:3px 8px;margin:3px 0;
                                    font-size:0.82em;background:#f9fafb;
            ...
            for c in courses:
                if c["exam_date"] == day:
                    st.markdown(
                        f"""
                        <div style="background:#ef476f;color:white;
...
def page_analytics(user: dict):
    ...
    fig3.add_hline(
        ...
        line_dash="dash", line_color="#ef476f",
    ...
    fig4.add_trace(go.Bar(
        ...
        y=pc2["completed"] / 60, marker_color="#06d6a0"))
    fig4.add_trace(go.Bar(
...
def page_study_mode(user: dict):
    ...
    st.caption("Focus timers for deep work.  Runs entirely in your browser.")

    mode = st.radio(
        "Select timer",
        ["🍅 Pomodoro (25 / 5)", "🧬 Ultradian (90 / 20)"],
        horizontal=True)
    ...
    st.markdown(
        f"**{label} Mode** — {focus_min} m focus / {break_min} m break")

    st.components.v1.html(_timer_html(focus_min, break_min), height=400)

    with st.expander("ℹ️ About these methods"):
        st.markdown("""
**🍅 Pomodoro** — 25 min focus, 5 min break.  After 4 rounds, take a
longer 15–30 min break.  Good for sustained attention tasks.

**🧬 Ultradian** — 90 min focus, 20 min break.  Matches the body's natural
activity–rest cycle.  Best for deep, complex problem-solving.""")


def _timer_html(focus_min: int, break_min: int) -> str:
    return f"""
    <style>
        ...
        * {{ margin:0; padding:0; box-sizing:border-box; }}
        .tc {{
            ...
            color:#e0e0e0; max-width:420px; margin:0 auto;
            box-shadow: 0 10px 40px -10px rgba(67,97,238,0.4);
        }}
        .td {{ font-size:5em; font-weight:700; letter-spacing:4px;
               margin:16px 0; color:#4cc9f0;
               text-shadow:0 0 20px rgba(76,201,240,0.3);
               transition:color 0.4s; }}
        .td.brk {{ color:#06d6a0; text-shadow:0 0 20px rgba(6,214,160,0.3); }}
        .sl {{ font-size:1.1em; font-weight:700; text-transform:uppercase;
               letter-spacing:3px; margin-bottom:8px; min-height:1.5em; }}
        .sl.f {{ color:#4cc9f0; }} .sl.b {{ color:#06d6a0; }}
        .sl.p {{ color:#ffd166; }}
        .br {{ display:flex; gap:12px; justify-content:center; margin-top:22px; }}
        .btn {{ padding:10px 28px; border:none; border-radius:10px;
                font-family:inherit; font-size:1em; font-weight:700;
                cursor:pointer; transition:transform 0.1s; }}
        .btn:hover {{ transform:scale(1.05); }}
        .btn:active {{ transform:scale(0.97); }}
        .btn:disabled {{ opacity:0.5; cursor:not-allowed; }}
        .bs {{ background:#4361ee; color:white; }}
        .bp {{ background:#ffd166; color:#1a1a2e; }}
        .bx {{ background:#ef476f; color:white; }}
        .pb {{ width:100%; height:6px; background:rgba(255,255,255,0.1);
               border-radius:3px; margin-top:18px; overflow:hidden; }}
        .pf {{ height:100%; border-radius:3px;
               transition:width 1s linear, background 0.4s;
               background:#4cc9f0; }}
        .pf.brk {{ background:#06d6a0; }}
        .sc {{ font-size:0.85em; color:#888; margin-top:14px; }}
    </style>
    <div class="tc">
        <div class="sl" id="sL">Ready</div>
        <div class="td" id="tD">{focus_min:02d}:00</div>
        <div class="pb"><div class="pf" id="pB" style="width:0%"></div></div>
        <div class="sc" id="sC">Sessions completed: 0</div>
        <div class="br">
    ...
    <script>
    const FS={focus_min*60},BS={break_min*60};
    ...
    function tk(){{
        ...
        if(rm<0){{
            ...
            if(!ib){{sn++;sC.textContent='Sessions completed: '+sn;
    ...
    function re(){{clearInterval(iv);iv=null;ib=false;rm=FS;ts=FS;sn=0;
        ...
        sC.textContent='Sessions completed: 0';
...
def page_profile(user: dict):
    ...
    st.markdown(
        f"""
        <div class="nova-user-row">
            <div class="nova-avatar">{initials(user['display_name'])}</div>
            <div class="meta">
    ...
    with st.form("profile_form"):
        ...
        current_code = user.get("country_code", "DE")
    ...
    with st.expander("⚠️ Danger zone"):
        ...
        if st.button("🗑️ Clear my study data", type="secondary"):
