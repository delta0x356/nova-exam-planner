"""
SQLite persistence layer for Nova Exam Planner.
==============================================

All tables are scoped by `user_id`, so multiple users share the same
database file without ever seeing each other's data.

Schema
------
users            : account credentials + profile
courses          : one row per course per user
constraints      : one row per user (1:1)
study_sessions   : one row per (course, date) tuple
"""
...
@contextmanager
def get_connection():
    """Yield a SQLite connection with foreign keys enabled and Row factory."""
    _ensure_dir()
...
def init_db():
    """Create tables + indexes if they don't exist.  Safe to run on every start."""
...
def create_user(username: str, email: Optional[str], password_hash: str,
                ...
                country_code: str = "DE") -> int:
    """Insert a new user and a default constraints row.  Returns the new user_id."""
    with get_connection() as conn:
        cur = conn.execute(
...
def upsert_course(user_id: int, name: str, exam_date: dt.date,
    ...
    """Insert or update a course.  Returns the course id."""
    with get_connection() as conn:
        if course_id:
            conn.execute(
                """UPDATE courses
...
def get_constraints(user_id: int) -> dict:
    ...
    if not row:
        # Shouldn't happen — users always get a default row on creation.
        return {
            "user_id": user_id,
            "weekly_hours": 30,
...
def list_sessions(user_id: int) -> list[dict]:
    """Return all sessions joined with their course name."""
    with get_connection() as conn:
        rows = conn.execute("""
...
def replace_sessions(user_id: int, sessions: list[dict]):
    """Drop all sessions for this user, then insert the given list.

    Each dict needs: course_id, session_date (date or iso str),
    planned_minutes, completed_minutes (optional, default 0).
    """
    with get_connection() as conn:
...
def clear_user_data(user_id: int):
    """Wipe courses + sessions (constraints stay at defaults)."""
    with get_connection() as conn:
        conn.execute("DELETE FROM study_sessions WHERE user_id = ?", (user_id,))
