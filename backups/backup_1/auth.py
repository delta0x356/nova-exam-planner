"""
Authentication for Nova Exam Planner.
====================================

Password hashing uses PBKDF2-HMAC-SHA256 from the Python stdlib (no
external crypto dependencies).  Stored format is ``salt$hash`` in hex.

All state lives in ``st.session_state``:
    user_id   — primary key into the users table
...
def register_user(username: str, email: str, password: str,
                  ...
                  country_code: str = "DE"):
    """Returns (user_id, error).  On success error is None."""
...
def current_user() -> Optional[dict]:
    """Fetch the current user fresh from the DB (None if not logged in)."""
